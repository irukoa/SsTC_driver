program driver
  ...
  use Utilities_Suite, only: test_kpath, test_kslice, test_crys_to_cart_cart_to_crys, test_kpath_length
  ...
  if (rank == 0) write (error_unit, "(A)") &
    "Test (1/4): Testing kpath utility."
  call test_kpath(error)
  if (rank == 0) then
    if (allocated(error)) error stop "Test (1/4): FAILED."
  endif
  if (rank == 0) write (error_unit, "(A)") "Test (1/4): PASSED."

  if (rank == 0) write (error_unit, "(A)") &
    "Test (2/4): Testing kslice utility."
  call test_kslice(error)
  if (rank == 0) then
    if (allocated(error)) error stop "Test (2/4): FAILED."
  endif
  if (rank == 0) write (error_unit, "(A)") "Test (2/4): PASSED."

  if (rank == 0) write (error_unit, "(A)") &
    "Test (3/4): Testing crys_to cart and cart_to_crys utilities."
  call test_crys_to_cart_cart_to_crys(error)
  if (rank == 0) then
    if (allocated(error)) error stop "Test (3/4): FAILED."
  endif
  if (rank == 0) write (error_unit, "(A)") "Test (3/4): PASSED."

  if (rank == 0) write (error_unit, "(A)") &
    "Test (4/4): Testing kpath_lenght utility."
  call test_kpath_length(error)
  if (rank == 0) then
    if (allocated(error)) error stop "Test (4/4): FAILED."
  endif
  if (rank == 0) write (error_unit, "(A)") "Test (4/4): PASSED."
